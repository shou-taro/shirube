...
export function SchemaTree({ objects, relationships, activeId, onSelect }: SchemaTreeProps) {
  ...
  // While filtering, force every matching schema open so results aren't hidden behind a
  // collapsed section.
  const schemaOpen = (schema: string): boolean => query !== '' || !collapsedSchemas.has(schema)
  ...
  return (
    <div ref={containerRef} className="relative">
      <Button
        ...
        className={cn('hover:bg-brand/15 hover:text-brand', open && 'bg-brand/15 text-brand')}
      >
        ...
        <div className="absolute right-0 top-full z-30 mt-1 flex max-h-[70vh] w-80 flex-col overflow-hidden rounded-lg border bg-popover text-popover-foreground shadow-md">
          <div className="flex items-center gap-2 border-b p-2">
            <input
              ...
              autoFocus
              className="h-7 w-full rounded-md border bg-background px-2 text-sm outline-none focus:ring-2 focus:ring-brand"
            />
            <button
              type="button"
              aria-label={t('tree.close')}
              onClick={() => setOpen(false)}
              className="flex size-7 shrink-0 items-center justify-center rounded-md text-muted-foreground hover:bg-brand/15 hover:text-brand"
            >
              <X className="size-4" />
            </button>
          </div>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-1">
            {groups.length === 0 ? (
              <p className="px-2 py-3 text-sm text-muted-foreground">{t('tree.noMatches')}</p>
            ) : (
              groups.map((group) => (
