...
# named ones. Reused across the three queries below via a shared WHERE fragment.
_SCHEMA_FILTER = (
    "n.nspname NOT IN ('pg_catalog', 'information_schema') "
    "AND n.nspname NOT LIKE 'pg_%%' "
    "AND (cardinality(%(schemas)s::text[]) = 0 OR n.nspname = ANY(%(schemas)s::text[]))"
)
...
_OBJECTS_SQL = f"""
    ...
    WHERE c.relkind IN ('r', 'v', 'm') AND {_SCHEMA_FILTER}
...
_RELATIONSHIPS_SQL = f"""
    ...
    WHERE con.contype = 'f' AND {_SCHEMA_FILTER}
...
def build_graph(
    ...
    foreign key into a schema that was not requested).
    ...
    return SchemaGraph(objects=objects, relationships=relationships)
...
class PostgresSchemaInspector:
    ...
    def inspect(self, params: ConnectionParams, schemas: Sequence[str]) -> SchemaGraph:
        """Read objects, columns and foreign keys from the database.
        ...
        return build_graph(object_rows, column_rows, relationship_rows)
