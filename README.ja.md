...
- ✏️ **自分でリレーションを引く** — 外部キーが宣言されていないと、本当は関連していても shirube はそのつながりを認識できません。ER 図に線が引かれず、関連テーブルをたどって移動することもできません。列と列を結べば、その足りないつながりを自分で補えます（点線で表示）。引いたリンクは接続ごとに保存され、データベース本体には触れません。
