...
- 🧭 **2 つのテーブルを結ぶ経路をたどる** — 行き先を選ぶと、今いるテーブルからそこまでの最短経路を小さな図で示します。経路上のテーブルを順に並べ、隣り合うテーブルは「どの列でつながるか」で結ぶので、順番だけでなくつながり方まで見えます。テーブルをクリックすると地図がそこへ移動し、経路を 1 つずつ辿れます。
- ✏️ **自分でリレーションを引く** — 外部キーが宣言されていないと、本当は関連していても shirube はそのつながりを認識できません。その場合は、列と列を結んで足りないつながりを補えます（点線で表示）。引いたリンクは接続ごとに保存され、データベース本体には触れません。
...
これまでに PostgreSQL と SQLite に対応し、スキーマを ER 図で表示する基本機能と AI ナビゲーターを作ってきました。画面は英語と日本語を切り替えられます。

この先は、MySQL への対応や、対応言語をさらに増やすことなどに取り組んでいく予定です。

## 🤝 コントリビュート

ソースからの起動方法、プロジェクト構成、テストやリントの実行方法は [`CONTRIBUTING.md`](CONTRIBUTING.md) にまとめています。
